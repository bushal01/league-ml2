...
def pull_match_history(player_id, region):
    logging.info('Pulling match history for player ' + str(player_id))
...
def pull_match(match_id, region):
    ...
    url = ('https://' + region + '.' + os.getenv('MATCH_URL') + str(match_id) + '?api_key=' + os.getenv('API_KEY'))
    return requests.get(url).json()


def api_request(request_type, rid, request_log, region='na1', request_error_log=[]):
    # Wait to make next request until under the rate limit
    time.sleep(rate_limit(request_log))
    request_log.append((rid, time.time()))

    # Make request
    r = API_REQUEST_FUNCTIONS[request_type](rid, region)
    ...
    if validate_request(r):
        ...
        return handle_invalid_request(r, request_type, rid, request_log, region, request_error_log)
...
def handle_invalid_request(r, request_type, rid, request_log, region, request_error_log):
    ...
    if r['status'] == '429':
        ...
        time.sleep(10)
        return api_request(request_type, rid, request_log, region, request_error_log)
    else:
        time.sleep(10)
        return api_request(request_type, rid, request_log, region, request_error_log)
